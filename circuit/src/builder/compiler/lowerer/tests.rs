...
fn test_witness_sharing() {
    ...
    // The `connect(c_42, p0)` value↔const pair aliases `p0` into the constant's slot (a bus
    // equality, no extra row); the value↔value chains (p1/p2/p3 and sum/p4) alias the same way.
