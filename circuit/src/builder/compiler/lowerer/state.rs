...
impl<'a, F: Field> LoweringState<'a, F> {
    ...
    pub fn new(lowerer: ExpressionLowerer<'a, F>) -> Result<Self, CircuitBuilderError> {
        ...
        Ok(Self {
            ...
            npo_registry,
            non_primitive_ops,
            // Build the DSU from all declared connect pairs.
            //
            // A `connect(a, b)` unions `a` and `b` into one witness slot. This is sound for
            // value↔value pairs (both compute the same value) AND for value↔const pairs (the
            // dominant case being `assert_zero(x)` == `connect(x, ExprId::ZERO)`): when the
            // shared slot was already CREATED by the constant's `Op::Const`, the value-bearing
            // op that targets it is demoted to a WitnessChecks READER, not a second creator
            // (see `generate_preprocessed_columns`: `out_already_defined ⇒ out_is_creator =
            // false`). The reader-receive binds the value to the constant's pinned slot — i.e.
            // it ENFORCES `x == c` as a bus equality with NO added row. At witness generation a
            // genuinely-nonzero `x` writing the shared slot fails as a `WitnessConflict` (a
            // correct rejection of a forged proof); the error names every `ExprId` sharing the
            // slot (see `CircuitRunner::witness_conflict`), and `dump_allocation_log()` gives the
            // full picture.
